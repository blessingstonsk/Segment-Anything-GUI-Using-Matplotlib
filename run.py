...
from tkinter import Tk, filedialog  # Import file dialog from tkinter
...
class Segmenter():
    def __init__(self, img):
        ...
        self.label = 1
        self.add_plot, = self.ax.plot([], [], 'o', markerfacecolor='white', markeredgecolor='teal', markersize=5)
        self.rem_plot, = self.ax.plot([], [], 'x', markerfacecolor='white', markeredgecolor='teal', markersize=5)
    ...
    def save_annotation(self):
        # Ask the user to select the directory for saving annotations
        save_dir = filedialog.askdirectory(title="Select Directory to Save Annotations")
        if save_dir:
            # Construct the file path for saving annotations
            labels_file_outpath = os.path.join(save_dir, "tag.jpg")
            # Save annotations
...
def run_gui():
    ...
    img_input_filepath = filedialog.askopenfilename(title="Select Image", filetypes=(("Image files", "*.png;*.jpg;*.jpeg;*.bmp"), ("All files", "*.*")))
    if img_input_filepath:
        ...
        plt.show(block=True)
        segmenter.save_annotation("tag.jpg")
